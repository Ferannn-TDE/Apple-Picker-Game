...
public class Basket : MonoBehaviour {
    ...
    void OnCollisionEnter(Collision coll) {
        ...
        if (collidedWith.CompareTag("Apple")) {
            ...
            Destroy(collidedWith);
            // Game Over when branch is caught - destroy all baskets
            ApplePicker apScript = Camera.main.GetComponent<ApplePicker>();
            if (apScript != null) {
                // Destroy all baskets to trigger game over
                foreach (GameObject basket in apScript.basketList) {
                    Destroy(basket);
                }
                apScript.basketList.Clear();
